...
const VERSION = 'v1'; // self es como el this, pero especifico para los serviceWorker
...
self.addEventListener('fetch', event => {
  const request = event.request; // get: Solo queremos hacer algo con las peticiones que son get, los post, put los delete no queremos trabajar con ellos.
...
async function cachedResponse(request) {
  const cache = await caches.open(VERSION); //.match Estamos preguntando al cache ya tienes una copia que le corresponde al request, si o no?. Si es no, nos va regresar undefine
